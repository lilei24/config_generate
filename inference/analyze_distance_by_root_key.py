#!/usr/bin/env python3
"""Analyze nearest same-top-key distance × root key vs generation metrics.

Two-level grouping:
  1. nearest_same_top_key_group  ("0", "1", "2", …, "inf")
  2. target_top_level_key        (gold answer's top-level config name)
"""
...
def collect_rows(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    total = len(files)
    print("[distance-by-root-key] start: %s files" % total, flush=True)
    ...
    for index, (split, task, result_path) in enumerate(files, start=1):
        ...
        # --- Read QA sample for graph input and answer ---
        ...
        # --- Read result record to get answer top-level keys ---
        ...
        # Only compute distance stats when answer has exactly one top-level key
...
def _progress(
    ...
    if interval > 0 and (index % interval == 0 or index == total):
        ...
        print(
            "[distance-by-root-key] %s/%s files (%.2f%%), %.2f files/s, eta %.1fs"
            % (
                index,
                total,
...
# ---------------------------------------------------------------------------
# Two-level grouping: nearest_same_top_key_group × target_top_level_key
# ---------------------------------------------------------------------------
...
def group_rows_two_level(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Group by (split, task, nearest_same_top_key_group, target_top_level_key)."""
    grouped: DefaultDict[
    ...
    for row in rows:
        key = (
            row["split"],
            row["task"],
            str(row.get("nearest_same_top_key_group", "")),
            str(row.get("target_top_level_key", "")),
        )
        grouped[key].append(row)

    output: List[Dict[str, Any]] = []
    for (split, task, dist_group, root_key), items in sorted(
        grouped.items(),
        key=lambda item: (
            item[0][0],
            item[0][1],
            _distance_sort_key(item[0][2]),
            item[0][3],
        ),
    ):
        ...
        output.append(
            {
                "split": split,
                "task": task,
                "nearest_same_top_key_group": dist_group,
...
def run(args: argparse.Namespace) -> None:
    ...
    write_csv(
        output_root / "per_file_distance_by_root_key.csv",
        strip_internal_fields(rows),
        PER_FILE_FIELDS,
    )

    grouped = group_rows_two_level(rows)
    write_csv(
        output_root / "distance_by_root_key_metrics.csv",
        grouped,
        GROUP_FIELDS,
    )
    ...
    write_json(
        output_root / "summary.json",
        {
            ...
            "grouping": (
                "Two-level grouping: "
                "1) nearest_same_top_key_group (actual shortest-path distances: 0, 1, 2, …, inf); "
                "2) target_top_level_key (gold answer's top-level config name / root key). "
                "Only samples whose gold answer has exactly one top-level key contribute "
                "distance stats and appear in non-empty distance groups."
            ),
        },
    )
    print(
        "[distance-by-root-key] done. output: %s" % output_root, flush=True
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze nearest same-top-key distance × root key vs metrics."
    )
    parser.add_argument(
