#!/usr/bin/env python3
"""Analyze betweenness centrality × nearest same-top-key distance vs generation metrics.

Groups samples first by betweenness centrality (0.1-width buckets), then within each
bucket sub-groups by actual nearest-same-top-key-distance values.
"""
...
# ---------------------------------------------------------------------------
# JSON / metric low-level utilities (inlined from metric.py)
# ---------------------------------------------------------------------------

def _load_json(x: Any) -> Any:
    """Accept a dict/list or a JSON string."""
    if isinstance(x, str):
        return json.loads(x)
    return x
...
def evaluate_json(pred: Any, gold: Any, array_mode: str = "wildcard") -> Dict[str, Any]:
    ...
    result["field_path"] = _counter_prf(pred_features["field_paths"], gold_features["field_paths"])
    result["leaf_triple"] = _counter_prf(pred_features["leaf_triples"], gold_features["leaf_triples"])
    result["value_accuracy"] = _value_accuracy_metric(pred_features, gold_features)
    result["field_name"] = _counter_prf(pred_features["field_names"], gold_features["field_names"])
    result["hallucination_missing"] = _hallucination_missing_metric(pred_features["field_paths"], gold_features["field_paths"])
    return result
...
def finalize_accumulator(acc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "value_accuracy": {
            ...
            "accuracy": value_counts["correct_value_count"] / matched_leaf_path_count if matched_leaf_path_count else 0.0,
        },
...
def collect_rows(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    total = len(files)
    print("[betweenness-by-distance] start: %s files" % total, flush=True)
    ...
    for index, (split, task, result_path) in enumerate(files, start=1):
        ...
        if len(target_keys) == 1:
            base.update(
                nearest_same_top_key_stats(input_value, base["target_node_id"], target_keys[0])
            )
...
def _progress(index: int, total: int, started_at: float, interval: int = DEFAULT_PROGRESS_INTERVAL) -> None:
    if interval > 0 and (index % interval == 0 or index == total):
        elapsed = max(0.001, time.time() - started_at)
        speed = index / elapsed
        eta = (total - index) / speed if speed > 0 else 0.0
        print(
            "[betweenness-by-distance] %s/%s files (%.2f%%), %.2f files/s, eta %.1fs"
...
def group_rows_two_level(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Group by (split, task, betweenness_centrality_group, nearest_same_top_key_distance)."""
...
def run(args: argparse.Namespace) -> None:
    ...
    # Per-file CSV
    write_csv(
        output_root / "per_file_betweenness_by_distance.csv",
        strip_internal_fields(rows),
        PER_FILE_FIELDS,
    )

    # Two-level grouped CSV
    grouped = group_rows_two_level(rows)
    write_csv(
        output_root / "betweenness_by_distance_metrics.csv",
        grouped,
        GROUP_FIELDS,
    )
    ...
    write_json(
        output_root / "summary.json",
        {
            ...
            "grouping": (
                ...
                "nearest_same_top_key_distance is computed only when the gold answer has exactly "
                "one top-level key; otherwise the distance field is empty."
            ),
            "neighbor_count_rule": (
    ...
    print("[betweenness-by-distance] done. output: %s" % output_root, flush=True)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze betweenness centrality × nearest same-top-key distance vs metrics."
    )
