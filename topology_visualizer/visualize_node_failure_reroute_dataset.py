...
let scale=1,taskOnly=false,allLabels=false,selected=null,selectedPath=0;
...
function render(){{
 ...
 data.edges.forEach(edge=>{{const a=nodes.get(edge.source),b=nodes.get(edge.target);if(!a||!b)return;const visible=edgeVisible(edge),start=linePoint(a,b,23),end=linePoint(b,a,23),answer=isPathEdge(edge);let cls="edge"+(answer?" answer":"")+(edge.isRemoved?" removed":"")+(selected===`e${{edge.index}}`?" selected":"")+(!visible?" dim":"");const line=make("line",{{x1:start.x,y1:start.y,x2:end.x,y2:end.y,class:cls}});line.addEventListener("click",()=>showEdge(edge));const tip=make("title");tip.textContent=`${{edge.source}} [${{edge.leftPort}}] ↔ ${{edge.target}} [${{edge.rightPort}}]${{edge.isRemoved?'\\n节点故障后移除':''}}${{edge.inAnswer?'\\n属于标准答案路径':''}}`;line.append(tip);edgesLayer.append(line);if(edge.isRemoved&&visible){{const x=(a.x+b.x)/2,y=(a.y+b.y)/2,s=7;marksLayer.append(make("line",{{x1:x-s,y1:y-s,x2:x+s,y2:y+s,class:"remove-x"}}),make("line",{{x1:x-s,y1:y+s,x2:x+s,y2:y-s,class:"remove-x"}}));}}}});
 const occupied=[];data.nodes.forEach(node=>occupied.push({{x:node.x-26,y:node.y-26,w:52,h:52}}));
 data.nodes.forEach(node=>{{const relevant=node.isSource||node.isTarget||node.isFailed||node.inAnswer;if(taskOnly&&!relevant)return;let cls="node"+(node.inAnswer?" answer":"")+(node.isSource?" source":"")+(node.isTarget?" target":"")+(node.isFailed?" failed":"")+(selected===`n${{node.id}}`?" selected":"");const group=make("g",{{class:cls,transform:`translate(${{node.x}} ${{node.y}})`}});group.addEventListener("click",()=>showNode(node));group.append(make("circle",{{r:20}}));const short=make("text",{{class:"short-id",y:1}});short.textContent=clip(node.id,11);group.append(short);if(node.isFailed){{const badge=make("text",{{class:"badge",y:-28}});badge.textContent="故障";group.append(badge);}}const tip=make("title");tip.textContent=`${{node.id}}\nNAME: ${{node.name}}\nTYPE: ${{node.type}}\nROLE: ${{node.role}}`;group.append(tip);nodesLayer.append(group);
   if(allLabels||relevant){{const full=node.name==="<missing>"?node.id:`${{node.name}} · ${{node.id}}`,text=clip(full,28),box=labelPosition(node,text,occupied),label=make("g",{{class:"node-label"}});if(Math.abs(box.dx)>1||Math.abs(box.dy-34)>1)label.append(make("line",{{x1:node.x,y1:node.y,x2:box.x+box.w/2,y2:box.y+box.h/2}}));label.append(make("rect",{{x:box.x,y:box.y,width:box.w,height:box.h}}));const value=make("text",{{x:box.x+box.w/2,y:box.y+box.h/2+1}});value.textContent=text;label.append(value);labelsLayer.append(label);}}
 }});applyScale();
}}
...
</style></head><body><header><h1>节点故障绕行数据集可视化</h1><div class="stats">样本 {len(records)} · 校验通过 {sum(not r.validation_issue_count for r in records)} · 含问题 {sum(bool(r.validation_issue_count) for r in records)}</div></header><main><div class="filters"><input id="q" type="search" placeholder="按文件名、节点 ID 筛选"><select id="split"><option value="">全部划分</option><option>train</option><option>val</option></select></div><div class="table"><table><thead><tr><th>文件</th><th>划分</th><th>源节点</th><th>目标节点</th><th>故障节点</th><th>跳数</th><th>答案路径数</th><th>失效边数</th><th>校验</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div></main><script>
