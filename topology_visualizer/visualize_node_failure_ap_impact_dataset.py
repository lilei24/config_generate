...
def parse_sample(path: Path, source_label: str) -> tuple[dict[str, Any], PageRecord]:
    ...
    raw_disconnected_ids = answer.get("disconnected_ap_ids")
    disconnected_ap_ids = normalize_node_ids(raw_disconnected_ids)
    if not isinstance(raw_disconnected_ids, list):
        raise ValueError("task_answer.disconnected_ap_ids 不是数组")
    disconnected_ap_set = set(disconnected_ap_ids)
    ...
    for node_id in disconnected_ap_ids:
        node = nodes_by_id.get(node_id)
        if node is None:
            issues["answer-ap-not-found"] += 1
        elif node["role"] != "AP":
            issues["answer-node-role-not-ap"] += 1
    if failed_node_id in disconnected_ap_set:
        issues["failed-node-listed-as-disconnected-ap"] += 1
    ...
    positions, width, height = initial_positions(
        sorted(nodes_by_id), adjacency, failed_node_id, disconnected_ap_set
    )
    ...
    for node_id, node in sorted(nodes_by_id.items()):
        ...
        node["isAffectedAp"] = node_id in disconnected_ap_set
        nodes.append(node)

    level = impact_level(len(disconnected_ap_ids))
    payload = {
        ...
        "answer": answer,
        "disconnectedApIds": disconnected_ap_ids,
        "impactLevel": level,
        "nodes": nodes,
        "edges": edges,
    ...
    record = PageRecord(
        ...
        disconnected_ap_count=len(disconnected_ap_ids),
        impact_level=level,
...
def main() -> None:
    ...
    summary = {
        ...
        "total_disconnected_aps": sum(record.disconnected_ap_count for record in records),
