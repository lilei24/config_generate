#!/usr/bin/env python3
"""从站点列表批量调用 OpenCode，完成两节点最短路径任务。
...
源节点和目标节点并构造提示词。OpenCode 的回答不会写回标准答案目录，而是先
复制对应的 with_answer 样本，再在输出文件中增加 ``opencode-answer`` 和
...
def build_prompt(site: str, sample: dict[str, Any]) -> str:
    source_node = sample.get("task_source_node_name")
    target_node = sample.get("task_target_node_name")
    if not isinstance(source_node, str) or not source_node:
        raise ValueError("样本缺少有效的 task_source_node_name")
    if not isinstance(target_node, str) or not target_node:
        raise ValueError("样本缺少有效的 task_target_node_name")

    return f"""你是网络物理拓扑分析 Agent。

你必须根据站点名称调用拓扑数据 Provider 获取拓扑，并使用
topograph_understand 提供的路径计算能力完成任务。不得根据设备名称猜测答案。

站点名称：{site}
任务：查找 {source_node} 到 {target_node} 的全部最短物理路径。
...
5. 最终只输出一个 JSON 对象，不输出解释、Markdown 代码块或思考过程。
...
def build_result_document(
    ...
    output["opencode-answer"] = result.answer
    output["opencode-run"] = {
