...
6. 节点图形只显示节点 ID 和 `DEVICEROLE`，不在拓扑区域显示设备名称；设备名称、TYPE、MODEL 和节点度数在点击节点后的详情区域展示。
7. 默认只显示源节点、目标节点、故障节点和答案路径节点的完整 `ID · DEVICEROLE` 标签；开启“显示完整标签”后，使用多候选位置和矩形碰撞检测减少标签遮挡。
8. 右侧固定展示任务 Question 和格式化 Answer，点击节点或链路时只更新顶部详情，不覆盖任务内容。
9. 生成逐样本 HTML、可筛选总索引和 `visualization_summary.json` 汇总。

页面支持完整拓扑/任务相关视图切换、节点拖拽、单条答案路径高亮、节点搜索、缩放、适配视图，以及节点和链路详情检查。所有资源均内嵌在 HTML 中，不依赖外部服务。
