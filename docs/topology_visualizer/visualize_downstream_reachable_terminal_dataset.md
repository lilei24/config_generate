...
- 蓝色节点：问题指定的核心上游节点。
- 绿色节点：`task_answer.downstream_leaf_node_ids` 中的下游终端叶子。
...
3. 校验答案节点存在、度数等于 1、自身不是 CORE/Firewall、能够到达所选上游，并确认没有距离更近或等距的同角色竞争上游。
...
输入必须包含 `nodes`、`links`、`task_upstream_node_id` 和 `task_answer.downstream_leaf_node_ids`。脚本只读取任务样本，不修改数据集。
