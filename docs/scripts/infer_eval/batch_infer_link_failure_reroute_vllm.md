# batch_infer_link_failure_reroute_vllm.py

> 代码位置：[`scripts/infer_eval/batch_infer_link_failure_reroute_vllm.py`](../../../scripts/infer_eval/batch_infer_link_failure_reroute_vllm.py)
...
| `--hidden-root HIDDEN_ROOT` | without_answer 数据集根目录，默认: link_failure_reroute_dataset/without_answer |
| `--answer-root ANSWER_ROOT` | with_answer 数据集根目录，默认: link_failure_reroute_dataset/with_answer |
| `--output-root OUTPUT_ROOT` | 推理结果根目录，默认: vllm-results/link_failure_reroute |
...
```bash
python scripts/infer_eval/batch_infer_link_failure_reroute_vllm.py --help
```
