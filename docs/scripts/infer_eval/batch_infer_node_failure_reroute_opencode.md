# batch_infer_node_failure_reroute_opencode.py

> 代码位置：[`scripts/infer_eval/batch_infer_node_failure_reroute_opencode.py`](../../../scripts/infer_eval/batch_infer_node_failure_reroute_opencode.py)
...
| `--hidden-root HIDDEN_ROOT` | without_answer 数据集根目录，默认: node_failure_reroute_dataset_from_raw/without_answer |
| `--answer-root ANSWER_ROOT` | with_answer 数据集根目录，默认: node_failure_reroute_dataset_from_raw/with_answer |
| `--output-root OUTPUT_ROOT` | 推理结果根目录，默认: opencode-results/node_failure_reroute |
...
```bash
python scripts/infer_eval/batch_infer_node_failure_reroute_opencode.py --help
```
