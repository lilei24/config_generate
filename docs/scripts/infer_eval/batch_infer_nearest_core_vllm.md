# batch_infer_nearest_core_vllm.py

> 代码位置：[`scripts/infer_eval/batch_infer_nearest_core_vllm.py`](../../../scripts/infer_eval/batch_infer_nearest_core_vllm.py)
...
| `--output-root OUTPUT_ROOT` | 推理结果根目录，默认: vllm-results/uplink_node_path |
...
```bash
python scripts/infer_eval/batch_infer_nearest_core_vllm.py --help
```
