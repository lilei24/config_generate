# batch_infer_node_neighborhood_reachability_vllm.py

> 代码位置：[`scripts/infer_eval/batch_infer_node_neighborhood_reachability_vllm.py`](../../../scripts/infer_eval/batch_infer_node_neighborhood_reachability_vllm.py)
...
| `--hidden-root HIDDEN_ROOT` | without_answer 数据集根目录，默认: node_neighborhood_reachability_dataset/without_answer |
| `--answer-root ANSWER_ROOT` | with_answer 数据集根目录，默认: node_neighborhood_reachability_dataset/with_answer |
| `--output-root OUTPUT_ROOT` | 推理结果根目录，默认: vllm-results/node_neighborhood_reachability |
...
```bash
python scripts/infer_eval/batch_infer_node_neighborhood_reachability_vllm.py --help
```
