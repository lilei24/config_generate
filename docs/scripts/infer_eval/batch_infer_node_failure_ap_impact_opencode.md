# batch_infer_node_failure_ap_impact_opencode.py

> 代码位置：[`scripts/infer_eval/batch_infer_node_failure_ap_impact_opencode.py`](../../../scripts/infer_eval/batch_infer_node_failure_ap_impact_opencode.py)
...
| `--hidden-root HIDDEN_ROOT` | without_answer 数据集根目录，默认: node_failure_ap_impact_dataset/without_answer |
| `--answer-root ANSWER_ROOT` | with_answer 数据集根目录，默认: node_failure_ap_impact_dataset/with_answer |
| `--output-root OUTPUT_ROOT` | 推理结果根目录，默认: opencode-results/node_failure_ap_impact |
...
```bash
python scripts/infer_eval/batch_infer_node_failure_ap_impact_opencode.py --help
```
