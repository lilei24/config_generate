# evaluate_vlan_constrained_shortest_path_results.py

> 代码位置：[`scripts/infer_eval/evaluate_vlan_constrained_shortest_path_results.py`](../../../scripts/infer_eval/evaluate_vlan_constrained_shortest_path_results.py)
...
| 指标 | 含义 |
|---|---|
| `vlan_id_accuracy` | 模型返回的 VLAN ID 是否与任务指定 VLAN 一致。 |
...
| 参数 | 说明 |
|---|---|
| `result_path` | 单个同时包含 task_answer 和 model-output 的结果 JSON，或包含 train/val 的结果目录，默认: vllm-results/vlan_constrained_shortest_path |
| `-h, --help` | 显示当前脚本的完整命令帮助后退出。 |
| `--output-dir OUTPUT_DIR` | 本地评估输出目录，默认: vllm-results/vlan_constrained_shortest_path-evaluation |
| `--split {train,val,all}` | 评估的数据划分，默认: val |
| `--progress-interval PROGRESS_INTERVAL` | 每处理 N 个文件打印进度，0 表示关闭，默认: 100 |
| `--swanlab-project SWANLAB_PROJECT` | SwanLab 项目名称，默认: topology-vlan-constrained-path |
| `--swanlab-experiment SWANLAB_EXPERIMENT` | SwanLab 实验名称，默认: vlan-constrained-path-evaluation |
...
| `DEFAULT_OUTPUT_DIR` | `'vllm-results/vlan_constrained_shortest_path-evaluation'` |

## 运行方式

```bash
python scripts/infer_eval/evaluate_vlan_constrained_shortest_path_results.py --help
```
