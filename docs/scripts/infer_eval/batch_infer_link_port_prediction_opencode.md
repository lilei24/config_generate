# batch_infer_link_port_prediction_opencode.py

> 代码位置：[`scripts/infer_eval/batch_infer_link_port_prediction_opencode.py`](../../../scripts/infer_eval/batch_infer_link_port_prediction_opencode.py)

## 功能与业务价值

使用 OpenCode 批量推理链路端口预测任务。
...
| `--hidden-root HIDDEN_ROOT` | without_answer 数据集根目录，默认: link_port_prediction_dataset/without_answer |
| `--answer-root ANSWER_ROOT` | with_answer 数据集根目录，默认: link_port_prediction_dataset/with_answer |
| `--output-root OUTPUT_ROOT` | 推理结果根目录，默认: opencode-results/link_port_prediction |
...
```bash
python scripts/infer_eval/batch_infer_link_port_prediction_opencode.py --help
```
