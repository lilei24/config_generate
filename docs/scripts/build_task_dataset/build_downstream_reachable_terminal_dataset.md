...
3. 将度数为 1、`device.TYPE` 为 AP 或 LSW，且自身角色不是 CORE/Firewall 的节点作为终端叶子候选。
...
7. 问题中写入所选上游节点 ID 和角色，`task_answer` 只包含按字典序排列的 `downstream_terminal_node_ids`。
...
| `task_question` | 下游可达终端查询问题、JSON Schema 和输出示例。 |
| `task_answer.downstream_terminal_node_ids` | 唯一归属于所选上游节点的全部 AP/LSW 终端节点 ID。 |
