...
1. 为每个 AP 按角色优先级寻找最高优先级的可达角色，并仅保留该角色中距离 AP 最近的上游节点；距离相同的节点全部保留。
2. 汇总各 AP 的最近上游候选，选择 AP 到候选目标最短距离最小的节点作为本图指定上游目标；距离相同时按节点 ID 稳定选择，不进行随机选择。
3. 枚举非 AP 故障节点并删除其关联边，判断各 AP 是否仍能到达正常目标。
4. 按影响 AP 数量划分 small、medium、large，每图默认各选择一个。
