# build_link_port_prediction_dataset.py

> 代码位置：[`scripts/build_task_dataset/build_link_port_prediction_dataset.py`](../../../scripts/build_task_dataset/build_link_port_prediction_dataset.py)

## 功能与业务价值

构造隐藏目标链路 LEFTPORT、RIGHTPORT 和 LABEL 的端口预测任务数据集。

**业务价值：** 根据拓扑与设备上下文预测链路两端物理端口，验证接口关系理解。
...
| `--output-root OUTPUT_ROOT` | 任务数据集输出目录，默认: link_port_prediction_dataset |
| `--splits SPLITS [SPLITS ...]` | 处理的数据划分，默认: train val |
| `--seed SEED` | 固定随机种子，默认: 20260804 |
...
| `DEFAULT_OUTPUT_ROOT` | `'link_port_prediction_dataset'` |
...
```bash
python scripts/build_task_dataset/build_link_port_prediction_dataset.py --help
```
