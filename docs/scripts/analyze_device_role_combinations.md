# analyze_device_role_combinations.py

> 代码位置：[`scripts/analyze_device_role_combinations.py`](../../scripts/analyze_device_role_combinations.py)
...
| `--exclude-cooccurring-roles ROLE [ROLE ...]` | 同时出现时排除该文件的角色集合，默认: AP CORE |
| `--split {train,val,all}` | 统计的数据划分，默认: all |
| `-o OUTPUT_DIR, --output-dir OUTPUT_DIR` | 统计结果目录，默认: /tmp/device_role_combination_analysis |
...
| `DEFAULT_OUTPUT_DIR` | `'/tmp/device_role_combination_analysis'` |
...
```bash
python scripts/analyze_device_role_combinations.py --help
```
...
| `role_sort_key (function)` | 核心内部接口 |
| `role_combination (function)` | 核心内部接口 |
| `combination_text (function)` | 核心内部接口 |
